...
import { fetchMemberships, fetchPlotById, submitGroupApplication } from '../api/groups';
...
const JoinPlotPage = () => {
  ...
  useEffect(() => {
    if (!loadingUser && !user) {
      const nextPath = encodeURIComponent(`/plots/${plotId}/join`);
      navigate(`/auth?next=${nextPath}`, { replace: true });
    }
  }, [plotId, loadingUser, navigate, user]);
  ...
  useEffect(() => {
    const loadPlot = async () => {
      ...
      try {
        ...
        setServerError(error.message || 'Unable to load plot information right now.');
      } finally {
  ...
  useEffect(() => {
    ...
    if (plot.submitted_by?.id === user.id) {
      navigate(`/groups/${plot.group}`, { replace: true });
      return;
    }
    let mounted = true;
    const checkMembership = async () => {
      try {
        const myMemberships = await fetchMemberships();
        const list = Array.isArray(myMemberships?.results) ? myMemberships.results : myMemberships;
        const alreadyMember = (list || []).some(
          (membership) => membership.group === plot.group && membership.status === 'active',
        );
        if (mounted && alreadyMember) {
          navigate(`/groups/${plot.group}`, { replace: true });
        }
      } catch (error) {
        // Non-fatal: worst case the user sees the join form despite already being a member.
      }
    };
    checkMembership();
    return () => {
      ...
      <InvestorLayout active="plots">
        <div className="flex min-h-[50vh] items-center justify-center">
          <div className="max-w-md rounded-3xl bg-white p-6 text-center shadow-card">
            <p className="text-base font-semibold text-slate-900">Plot unavailable</p>
    ...
    <InvestorLayout active="plots">
      <div className="mx-auto w-full max-w-5xl">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.35em] text-primary">Request to join this plot</p>
            <h1 className="mt-2 text-2xl font-semibold text-slate-900">{plot?.name || 'Plot'}</h1>
            <p className="mt-2 text-sm text-slate-600">
              Complete the diligence checklist to request a spot. Our team reviews every request before the group forms.
            </p>
          </div>
          <Link
            to="/plots"
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
          <form onSubmit={handleSubmit} className="rounded-3xl border border-slate-100 bg-white p-6 shadow-card" noValidate>
            <div className="grid gap-5 md:grid-cols-2">
              <div>
                <label className="text-xs font-semibold uppercase tracking-wide text-slate-500" htmlFor="jp-name">
                  Full name
                </label>
                <input
                  id="jp-name"
              ...
              <div>
                <label className="text-xs font-semibold uppercase tracking-wide text-slate-500" htmlFor="jp-email">
                  Email
                </label>
                <input
                  id="jp-email"
            ...
            <div className="mt-5 grid gap-5 md:grid-cols-2">
              <div>
                <label className="text-xs font-semibold uppercase tracking-wide text-slate-500" htmlFor="jp-phone">
                  Phone number
                </label>
                <input
                  id="jp-phone"
              ...
              <div>
                <label className="text-xs font-semibold uppercase tracking-wide text-slate-500" htmlFor="jp-location">
                  Location / city
                </label>
                <input
                  id="jp-location"
            ...
            <div className="mt-5 grid gap-5 md:grid-cols-2">
              <div>
                <label className="text-xs font-semibold uppercase tracking-wide text-slate-500" htmlFor="jp-occupation">
                  Occupation
                </label>
                <select
                  id="jp-occupation"
              ...
              <div>
                <label className="text-xs font-semibold uppercase tracking-wide text-slate-500" htmlFor="jp-id">
            ...
            <div className="mt-5">
              <label className="text-xs font-semibold uppercase tracking-wide text-slate-500" htmlFor="jp-motivation">
                Why this plot?
              </label>
              <textarea
                id="jp-motivation"
                ...
                onChange={handleChange}
                placeholder="Describe your investment focus, capital readiness, or the impact you want to achieve."
                className={clsx(
            ...
            <div className="mt-6 grid gap-4 rounded-3xl border border-slate-100 bg-slate-50 p-4 text-sm text-slate-600 md:grid-cols-2">
              ...
              <label className="flex items-center gap-3">
                <input
                  ...
                  className="h-4 w-4 rounded border-slate-300 text-primary focus:ring-primary"
                />
                I understand the refundable onboarding deposit.
              </label>
              <label className="flex items-center gap-3">
                <input
                  type="checkbox"
                  ...
                  className="h-4 w-4 rounded border-slate-300 text-primary focus:ring-primary"
                />
                I commit to this plot's minimum ticket size.
              </label>
            </div>
            ...
            <div className="mt-6 flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
              ...
              />
              <label htmlFor="jp-consent" className="text-xs text-slate-600">
          ...
          <aside className="space-y-4">
            <div className="rounded-3xl border border-primary/20 bg-primary/5 p-5 text-sm text-slate-600 shadow-card">
              <p className="text-sm font-semibold text-primary">Plot snapshot</p>
              <div className="mt-3 space-y-2 text-xs">
                <p>
                  <span className="font-semibold text-slate-900">Location:</span> {plot?.plot_location}
                </p>
                <p>
                  <span className="font-semibold text-slate-900">Plot size:</span> {plot?.plot_size_sqm} m&sup2;
                </p>
                <p>
                  <span className="font-semibold text-slate-900">Land status:</span> {plot?.land_registration_status_label}
                </p>
                <p>
                  <span className="font-semibold text-slate-900">Construction cost:</span> {formatRwf(plot?.building_cost)}
                </p>
                <p>
                  <span className="font-semibold text-slate-900">Min investment:</span> {formatRwf(plot?.min_investment)}
                </p>
                <p>
            ...
            <div className="rounded-3xl border border-slate-100 bg-slate-50 p-5 text-xs text-slate-600">
              ...
              <ol className="mt-2 space-y-2">
                ...
                <li>2. On approval, you're added to the plot's investment group as a member.</li>
                <li>3. You receive chat access, governance tools, and investor status.</li>
              </ol>
            </div>
          </aside>
        </div>
